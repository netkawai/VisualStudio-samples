                                         ...
                                         void* pIndices, SDKMESH_CALLBACKS11* pLoaderCallbacks )
{
    ...
    bufferDesc.CPUAccessFlags = 0;
    bufferDesc.MiscFlags = 0;
    bufferDesc.StructureByteStride = (pHeader->IndexType == IT_16BIT) ? 2 : 4;
                               ...
                               UINT iSpecularSlot )
{
    ...
    for( UINT subset = 0; subset < pMesh->NumSubsets; subset++ )
    {
		...
		if ( m_PreviousPrimType != pSubset->PrimitiveType )
		{
			m_PreviousPrimType = pSubset->PrimitiveType;
			PrimType = GetPrimitiveType11( ( SDKMESH_PRIMITIVE_TYPE )pSubset->PrimitiveType );
			if( bAdjacent )
			{
				switch( PrimType )
        ...
        if( iDiffuseSlot != INVALID_SAMPLER_SLOT && m_pPreviousDiffuseTexture != pMat->pDiffuseRV11 && !IsErrorResource( pMat->pDiffuseRV11 ) )
        {
			m_pPreviousDiffuseTexture = pMat->pDiffuseRV11;
			pd3dDeviceContext->PSSetShaderResources( iDiffuseSlot, 1, &pMat->pDiffuseRV11 );
		}
        if( iNormalSlot != INVALID_SAMPLER_SLOT && m_pPreviousNormalTexture != pMat->pNormalRV11 && !IsErrorResource( pMat->pNormalRV11 ) )
        {
			m_pPreviousNormalTexture = pMat->pNormalRV11;
			pd3dDeviceContext->PSSetShaderResources( iNormalSlot, 1, &pMat->pNormalRV11 );
		}
        if( iSpecularSlot != INVALID_SAMPLER_SLOT && m_pPreviousSpecularTexture != pMat->pSpecularRV11 && !IsErrorResource( pMat->pSpecularRV11 ) )
        {
			m_pPreviousSpecularTexture = pMat->pSpecularRV11;
...
void CDXUTSDKMesh::ResetDirtyStateChecks()
{
	m_PreviousPrimType = 0xffffffff;
	m_pPreviousDiffuseTexture = 0;
	m_pPreviousNormalTexture = 0;
	m_pPreviousSpecularTexture = 0;
}

...
static const wchar_t *FXC_PATH_STRING_INSTALLED_WIN8SDK = L"\\Windows Kits\\8.0\\bin\\x86\\fxc.exe";
static const wchar_t *FXC_PATH_STRING_INSTALLED_DXSDK   = L"Utilities\\bin\\x86\\fxc.exe";  // DXSDK_DIR has a backslash on the end, so don't need one at the beginning here
                          ...
                          const GENERATE_ISA_TYPE i_keGenerateShaderISAType, const SHADER_COMPILER_EXE_TYPE i_keShaderCompilerExeType )
{
	...
	{
		// search for fxc in the win8 sdk
		...
		SHGetSpecialFolderPath(
			...
			CSIDL_PROGRAM_FILES, 
			FALSE );
		swprintf_s( m_wsFxcExePath, L"%s%s", wsProgramFilesDir, FXC_PATH_STRING_INSTALLED_WIN8SDK );
		if( !CheckFXC() )
		{
			// if you don't find fxc in the win8 sdk, search for it in the legacy dxsdk
			size_t requiredSize;
			wchar_t wsDxSdkDir[MAX_PATH] = {L'\0'};
			_wgetenv_s(&requiredSize, wsDxSdkDir, MAX_PATH, L"DXSDK_DIR");
			swprintf_s( m_wsFxcExePath, L"%s%s", wsDxSdkDir, FXC_PATH_STRING_INSTALLED_DXSDK );
		}
	...
	if( !CheckFXC() )
	{
		// For SHADER_COMPILER_EXE_INSTALLED, fxc.exe is looked for first in the default Win8 SDK install location, and then in the legacy DXSDK
			...
			while( *pTemp != L'\n' )
			{
			  ...
			  else if( !wcsncmp( pTemp, szNumGPRs, wcslen( szNumGPRs ) ) )
			  {
				  ...
				  if( pStr )
				  {
					  ...
					  io_uNumSGPR = (~0); // Special Flag to indicate that this is a GPR Pool
				  }
			  }
...
void ShaderCache::RenderISAInfo( CDXUTTextHelper* g_pTxtHelper, int iFontHeight, D3DXCOLOR FontColor, const Shader *i_pShaderCmp, wchar_t *o_wsGPRInfo )
{
    wchar_t wsOverallProgress[m_uPATHNAME_MAX_LENGTH];
	D3DXCOLOR RedFontColor( 1.0f, 0.0f, 0.0f, 1.0f );
	D3DXCOLOR GreenFontColor( 0.0f, 1.0f, 0.0f, 1.0f );
	D3DXCOLOR BlueFontColor( 0.0f, 0.0f, 1.0f, 1.0f );
...
static void fwriteString( const wchar_t* i_wcsString, FILE* i_pFile )
{
	assert( i_pFile );
	fwrite( i_wcsString, wcslen(i_wcsString) * sizeof(wchar_t), 1, i_pFile );
}
...
// Renders the progress of the shader generation process
//--------------------------------------------------------------------------------------
void ShaderCache::RenderShaderErrors( CDXUTTextHelper* g_pTxtHelper, int iFontHeight, D3DXCOLOR FontColor, const unsigned int ki_FrameTimeout )
{
    wchar_t wsOverallProgress[m_uPATHNAME_MAX_LENGTH];

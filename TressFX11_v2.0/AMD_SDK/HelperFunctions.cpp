...
//
// Various helper functions useful in many sampes
//
...
void AMD::CaptureFrame( ID3D11Texture2D* pCaptureTexture, WCHAR* pszCaptureFileName )
{
	...
	if( TexDesc.SampleDesc.Count > 1 )
	{
		...
		D3DX11SaveTextureToFile( DXUTGetD3D11DeviceContext(), pCaptureTexture, D3DX11_IFF_BMP, pszCaptureFileName );
...
void AMD::CreateCube( float fSize, ID3D11Buffer** ppVertexBuffer, ID3D11Buffer** ppIndexBuffer )
{
	if ( ppVertexBuffer )
	{
		...
		for ( int i = 0; i < 6; i++ )
		{
			...
			D3DXVECTOR3 normal = normalDirs[ i ];

    ...
    public class AuthenticationService
    {
        ...
        private async Task AcquireTokenForUserAsync()
        {
            ...
            if (accounts.Any())
            {
                try
                {
                    var authResult = await _identityClientApp.AcquireTokenSilent(Scopes, accounts.First()).ExecuteAsync(); 
            ...
            if (!IsConnected)
            {
                var authResult = await _identityClientApp.AcquireTokenInteractive(Scopes).ExecuteAsync();
